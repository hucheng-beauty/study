...
func main() {
	...
	outFile, err := parser.ParseFile(fileSet, astOutFilename,
		"package "+file.Name.Name, parser.ParseComments)
	// 遍历 AST，寻找接口
			...
			for _, spec := range genDecl.Specs {
				if typeSpec, ok := spec.(*ast.TypeSpec); ok {
					if interfaceType, ok := typeSpec.Type.(*ast.InterfaceType); ok {
						GenInterfaceDefaultImpl(typeSpec.Name.Name, interfaceType, outFile)
					}
				}
			}
		}
	...
	if err := SaveAstToFile(astOutFilename, fileSet, outFile); err != nil {
		log.Panicln(err.Error())
	}
}

// SaveAstToFile 将格式化后的代码保存到文件
func SaveAstToFile(fileName string, fileSet *token.FileSet, node *ast.File) error {
...
func GenInterfaceDefaultImpl(name string, interfaceType *ast.InterfaceType, file *ast.File) {
	type structNameDefaultImpl struct{}
	...
	for _, method := range interfaceType.Methods.List {
		...
		// 创建方法的函数体, BlockStmt 节点表示一个大括号的语句列表
		body := &ast.BlockStmt{
			List: []ast.Stmt{
